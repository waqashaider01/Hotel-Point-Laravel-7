    ...
    <div class="container-fluid">
          <div class="listcard " >
         	<div class="" style=" background-color:#48BBBE;margin-top:-2.1% !important;color:white; margin-bottom:15px;with:100%;margin-left:-2.1% !important;margin-right:-2.1%;border-radius:5px 5px 0px 0px;" >
		       <h2 class="pt-7 pl-3 pb-7" >Cancellation Room Nights Report</h2>
      ...
      <div wire:ignore class="row">
            <div class="col">
                   <div class="listcard listcard-custom mt-10 " style="height:350px;" >
                        <div id="nightscancel_chart" style="width: 100%; height: 100%;"></div>
                   </div>
             </div>
	   </div>
   	   ...
   	   <div  class="listcard listcard-custom mt-10 mb-4">
             ...
             <div class="table-responsive">
                <table  class="table " >
                    ...
                    <tbody id="tabbody">
                    @forelse($nightsCancelled as $row)
                        <tr class="text-center thin-bottom-border bg-white">
                            
                                <td><div class="idcolor" >{{$row[0]}}</div></td>
                                <td>{{$row[1]}}</td>
                                <td>{{$row[2]}}</td>
                                <td>{{$row[3]}}</td>
                                <td>{{$row[4]}}</td>
                        ...
                        @empty
                          <tr class="mytr text-center" >
                            <td colspan="13">No room type found</td>
                          </tr>
                    @endforelse
                    </tbody>
                </table>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function(){
            ...
            fetch("{{route('getMinDate')}}").then(result=> result.json()).then(json => show1(json))
                ...
                populateChart();
                document.addEventListener("cnChanged", () => {
...
function populateChart(){
          ...
          var chart = new google.visualization.AreaChart(document.getElementById('nightscancel_chart'));
