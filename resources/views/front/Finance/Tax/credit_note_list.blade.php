...
@section('content')
    <div>
        <x-table title='Credit Note List'>
            <x-slot name="header">
                <div class="row">
                    <div class="col-md-10 form-group d-inline-block">
                        <form action="{{route('credit-note-list')}}" method="get">
                        <div class="row">
                                <div class="col-md-3">
            ...
            @foreach($documents as $item)
                <tr>
                    <td><a href="{{asset('storage/invoices/'.$item->print_path)}}" style="color: #1BC5BD" download>{{$item->id}}</a></td>
                    <td>{{$item->mark_id}}</td>
                    ...
                    <td>{{$item->document_info->company->name}}</td>
