...
@section('content')
    <div>
        <x-table title='Cancellation fee List'>
            <x-slot name="header">
                <div class="row">
                    <div class="col-md-10 form-group d-inline-block">
                        <form action="{{route('cancellation-fee-list')}}" method="get">
                        <div class="row">
                                <div class="col-md-3">
            ...
            @foreach($documents as $item)
                <tr>
                    ...
                    <td>
                        <span style='color:red; font-weight:bold;'>{{(checkNoShow($item->activities->first()->reservation_id))?'No Show':''}}</span>
                    </td>
                </tr>
            @endforeach
        </x-table>
