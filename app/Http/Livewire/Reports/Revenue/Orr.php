...
class Orr extends Component
{
    ...
    {
        $this->computeData();
        $this->dispatchBrowserEvent('orrChanged');
        return view('livewire.reports.revenue.orr');
    }
    ...
    public function computeData(){
        ...
        $cdate=$this->date->day;
        $dataArray=[];
        $firstRow=["Month", "Other Room Revenue", "Other Room Revenue"];
        $jandata=["Jan" ];
        $febdata=["Feb" ];
        $marchData=["Mar" ];
        $aprData=["Apr" ];
        ...
        for ($i=1; $i <=12 ; $i++) {
            ...
            $monthlytotal=Document::join('document_types', 'documents.document_type_id', '=', 'document_types.id')
                                    ->join('activities', 'documents.id', '=', 'activities.document_id')
                                    ->join('reservations', 'reservations.id', '=', 'activities.reservation_id')
                                    ->whereBetween('documents.print_date', [$startDate, $endDate])
