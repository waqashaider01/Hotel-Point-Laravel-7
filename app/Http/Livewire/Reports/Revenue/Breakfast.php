...
class Breakfast extends Component
{
    ...
    {
        $this->computeData();
        $this->dispatchBrowserEvent('breakfastChanged');
        return view('livewire.reports.revenue.breakfast');
    }
    ...
    public function computeData(){
        ...
        $cdate=$this->date->day;
        $dataArray=[];
        $firstRow=["Month", "Breakfast Revenue", "Breakfast Revenue"];
        $jandata=["Jan" ];
        $febdata=["Feb" ];
        $marchData=["Mar" ];
        $aprData=["Apr" ];
        ...
        for ($i=1; $i <=12 ; $i++) {
            ...
            $monthrevenue=DailyRate::join('reservations', 'reservations.id', '=', 'daily_rates.reservation_id')
                                    ...
                                    ->where('room_types.type_status', '=', '1')->get();
                foreach($monthrevenue as $revenue){
                 
                            $derivedValue=$revenue['charge_type'];
                            $actualCheckin=$revenue['arrival_date'];
                            $dailyrateDate=$revenue['date'];
                            $dailyrate=$revenue['price'];
                            if ($dailyrateDate<$actualCheckin) {}
                            else{

                                    if ($derivedValue==0) {
                                    $derivedpercentage=$revenue['charge_percentage'];
                                    $netPercent=1.00+($derivedpercentage/100);
                                    $netValue=((float)$dailyrate/(float)$netPercent);
                                    $reservationBreakfast=$dailyrate-$netValue;
                                    
                                    
                                    }else{
                                        $reservationBreakfast=$revenue['charge_percentage'];
                                        
                                    }
                                $reservationBreakfast=number_format((float)$reservationBreakfast, 2, '.', '');
                                $monthlytotal+=(float)$reservationBreakfast;
