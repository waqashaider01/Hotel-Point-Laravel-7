...
class Fb extends Component
{
    ...
    public $otherCostOfSaleMTD;
    public $otherCostOfSaleYTD;
    public $totalMtdActualOE, $totalMtdBudgetOE, $totalMtdActualOOD, $totalMtdBudgetOOD, $totalYtdActualOE, $totalYtdBudgetOE, $totalYtdActualOOD, $totalYtdBudgetOOD;
    public $totalmtdValue, $totalmtdColor, $totalytdValue, $totalytdColor, $totalOEmtdVAlue, $totalOEmtdColor, $totalOEytdVAlue, $totalOEytdColor;
    ...
    public function render()
    {
        ...
        $totalYtdActualOOD=0;
        $totalYtdBudgetOOD=0;
        $getOEDescription=Description::where('category_id', 2)->get();
        ...
        $getOODDescription=Description::where('category_id', 3)->get();
        foreach ($getOODDescription as $description) {
             $opexMonth=OpexData::where('hotel_settings_id', getHotelSettings()->id)->where('description_id', $description->id)->whereBetween('date', [$startOfMonth, $this->selectedDate])->sum('amount');
             $opexYear=OpexData::where('hotel_settings_id', getHotelSettings()->id)->where('description_id', $description->id)->whereBetween('date', [$startOfYear, $this->selectedDate])->sum('amount');
             $getBudget=HotelBudget::where('hotel_settings_id', getHotelSettings()->id)->where('budget_year', $year)->where('sub_category', $description->id)->get();
             if ($getBudget->isEmpty()) {
                ...
                $budgetYear=(float)$getBudget['january']+(float)$getBudget['february']+(float)$getBudget['march']+(float)$getBudget['april']+(float)$getBudget['may']+(float)$getBudget['june']+(float)$getBudget['july']+(float)$getBudget['august']+(float)$getBudget['september']+(float)$getBudget['october']+(float)$getBudget['november']+(float)$getBudget['december'];
                $totalMtdBudgetOOD+=(float)$budgetMonth;
                $totalYtdBudgetOOD+=(float)$budgetYear;

                
            }
             $totalMtdActualOOD+=(float)$opexMonth;
             $totalYtdActualOOD+=(float)$opexYear;
             $monthdifference=$this->calculateDiff($budgetMonth, $opexMonth);
             $monthdifferenceValue=$monthdifference['difference'];
             $monthcolor=$monthdifference['color'];
             $yeardifference=$this->calculateDiff($budgetYear, $opexYear);
             $yeardifferenceValue=$yeardifference['difference'];
             $yearcolor=$yeardifference['color'];
             array_push($otherCostOfSaleMTD, [$description->name, $opexMonth, $budgetMonth, [$monthdifferenceValue, $monthcolor] ]);
             array_push($otherCostOfSaleYTD, [$description->name, $opexYear, $budgetYear, [$yeardifferenceValue, $yearcolor]]);
             
        }

          $totalOODmtdDifference=$this->calculateDiff($totalMtdBudgetOOD, $totalMtdActualOOD);
          $this->totalmtdValue=$totalOODmtdDifference['difference'];
          $this->totalmtdColor=$totalOODmtdDifference['color'];
          $totalOODytdDifference=$this->calculateDiff($totalYtdBudgetOOD, $totalYtdActualOOD);
          $this->totalytdValue=$totalOODytdDifference['difference'];
          $this->totalytdColor=$totalOODytdDifference['color'];
         

          $this->operatingExpenseMTD=$operatingExpenseMTD;
          $this->operatingExpenseYTD=$operatingExpenseYTD;
          $this->otherCostOfSaleMTD=$otherCostOfSaleMTD;
          $this->otherCostOfSaleYTD=$otherCostOfSaleYTD;
          ...
          $this->totalMtdActualOOD=$totalMtdActualOOD;
          $this->totalMtdBudgetOOD=$totalMtdBudgetOOD;
          $this->totalYtdActualOOD=$totalYtdActualOOD;
          $this->totalYtdBudgetOOD=$totalYtdBudgetOOD;
          $this->selectedDate=$this->selectedDate->toDateString();
          

          $this->dispatchBrowserEvent('fbChanged');

        return view('livewire.reports.opex.fb');
    }
