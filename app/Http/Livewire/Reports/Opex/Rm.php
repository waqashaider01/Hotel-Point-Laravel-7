...
class Rm extends Component
{
    ...
    public $energyCostMTD;
    public $energyCostYTD;
    public $totalMtdActualOE, $totalMtdBudgetOE, $totalMtdActualEC, $totalMtdBudgetEC, $totalYtdActualOE, $totalYtdBudgetOE, $totalYtdActualEC, $totalYtdBudgetEC;
    ...
    public function render()
    {
        ...
        $totalYtdActualEC=0;
        $totalYtdBudgetEC=0;
        $getOEDescription=Description::where('category_id', 9)->get();
        ...
        $getECDescription=Description::where('category_id', 10)->get();
        foreach ($getECDescription as $description) {
             $opexMonth=OpexData::where('hotel_settings_id', getHotelSettings()->id)->where('description_id', $description->id)->whereBetween('date', [$startOfMonth, $this->selectedDate])->sum('amount');
             $opexYear=OpexData::where('hotel_settings_id', getHotelSettings()->id)->where('description_id', $description->id)->whereBetween('date', [$startOfYear, $this->selectedDate])->sum('amount');
             $getBudget=HotelBudget::where('hotel_settings_id', getHotelSettings()->id)->where('budget_year', $year)->where('sub_category', $description->id)->get();
             if ($getBudget->isEmpty()) {
                ...
                $budgetYear=(float)$getBudget['january']+(float)$getBudget['february']+(float)$getBudget['march']+(float)$getBudget['april']+(float)$getBudget['may']+(float)$getBudget['june']+(float)$getBudget['july']+(float)$getBudget['august']+(float)$getBudget['september']+(float)$getBudget['october']+(float)$getBudget['november']+(float)$getBudget['december'];
                $totalMtdBudgetEC+=(float)$budgetMonth;
                $totalYtdBudgetEC+=(float)$budgetYear;

                
            }
             $totalMtdActualEC+=(float)$opexMonth;
             $totalYtdActualEC+=(float)$opexYear;
             $monthdifference=$this->calculateDiff($budgetMonth, $opexMonth);
             $monthdifferenceValue=$monthdifference['difference'];
             $monthcolor=$monthdifference['color'];
             $yeardifference=$this->calculateDiff($budgetYear, $opexYear);
             $yeardifferenceValue=$yeardifference['difference'];
             $yearcolor=$yeardifference['color'];
             array_push($energyCostMTD, [$description->name, $opexMonth, $budgetMonth, [$monthdifferenceValue, $monthcolor] ]);
             array_push($energyCostYTD, [$description->name, $opexYear, $budgetYear, [$yeardifferenceValue, $yearcolor]]);
             
        }

          $totalECmtdDifference=$this->calculateDiff($totalMtdBudgetEC, $totalMtdActualEC);
          $this->totalmtdValue=$totalECmtdDifference['difference'];
          $this->totalmtdColor=$totalECmtdDifference['color'];
          $totalECytdDifference=$this->calculateDiff($totalYtdBudgetEC, $totalYtdActualEC);
          $this->totalytdValue=$totalECytdDifference['difference'];
          $this->totalytdColor=$totalECytdDifference['color'];
         

          $this->operatingExpenseMTD=$operatingExpenseMTD;
          $this->operatingExpenseYTD=$operatingExpenseYTD;
          $this->energyCostMTD=$energyCostMTD;
          $this->energyCostYTD=$energyCostYTD;
          ...
          $this->totalMtdActualEC=$totalMtdActualEC;
          $this->totalMtdBudgetEC=$totalMtdBudgetEC;
          $this->totalYtdActualEC=$totalYtdActualEC;
          $this->totalYtdBudgetEC=$totalYtdBudgetEC;
          $this->selectedDate=$this->selectedDate->toDateString();

          $this->dispatchBrowserEvent('rmChanged');
        return view('livewire.reports.opex.rm');
    }
