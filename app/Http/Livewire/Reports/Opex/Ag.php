...
{
    public $selectedDate;
    public $agOperatingExpenseMTD;
    public $agOperatingExpenseYTD;
    public $totalMtdActualAG, $totalMtdBudgetAG, $totalYtdActualAG, $totalYtdBudgetAG;
    public $totalAGmtdVAlue, $totalAGmtdColor, $totalAGytdVAlue, $totalAGytdColor;
    ...
    public function render()
    {
        ...
        $agOperatingExpenseMTD=[];
        $agOperatingExpenseYTD=[];
        $totalMtdActualAG=0;
        $totalMtdBudgetAG=0;
        $totalYtdActualAG=0;
        $totalYtdBudgetAG=0;
        $getOEDescription=Description::where('category_id', 6)->get();
        foreach ($getOEDescription as $description) {
             ...
             if ($getBudget->isEmpty()) {
                ...
                $totalMtdBudgetAG+=(float)$budgetMonth;
                $totalYtdBudgetAG+=(float)$budgetYear;
             ...
             array_push($agOperatingExpenseMTD, [$description->name, $opexMonth, $budgetMonth, [$monthdifferenceValue, $monthcolor] ]);
             array_push($agOperatingExpenseYTD, [$description->name, $opexYear, $budgetYear, [$yeardifferenceValue, $yearcolor]]);
             $totalMtdActualAG+=(float)$opexMonth;
             $totalYtdActualAG+=(float)$opexYear;
        }

        $totalOEmtdDifference=$this->calculateDiff($totalMtdBudgetAG, $totalMtdActualAG);
        $this->totalAGmtdVAlue=$totalOEmtdDifference['difference'];
        $this->totalAGmtdColor=$totalOEmtdDifference['color'];
        $totalOEytdDifference=$this->calculateDiff($totalYtdBudgetAG, $totalYtdActualAG);
        $this->totalAGytdVAlue=$totalOEytdDifference['difference'];
        $this->totalAGytdColor=$totalOEytdDifference['color'];

         

          $this->agOperatingExpenseMTD=$agOperatingExpenseMTD;
          $this->agOperatingExpenseYTD=$agOperatingExpenseYTD;
          $this->totalMtdActualAG=$totalMtdActualAG;
          $this->totalMtdBudgetAG=$totalMtdBudgetAG;
          $this->totalYtdActualAG=$totalYtdActualAG;
          $this->totalYtdBudgetAG=$totalYtdBudgetAG;
          $this->selectedDate=$this->selectedDate->toDateString();
          $this->dispatchBrowserEvent('agChanged');
        return view('livewire.reports.opex.ag');
    }
